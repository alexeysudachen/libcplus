...
typedef struct _C_BLOWFISH
  {
    uint_t P[16 + 2];
    uint_t S[4][256];
  } C_BLOWFISH;

#ifdef _C_BLOWFISH_BUILTIN
...
static uint_t Blowfish_S[4][256] = 
  {
...
void Blowfish_Encrypt8_(C_BLOWFISH *self, uint_t *Lx, uint_t *Rx)
#ifdef _C_BLOWFISH_BUILTIN
  {
    uint_t  L = *Lx;
    uint_t  R = *Rx;
...
void Blowfish_Encrypt8(C_BLOWFISH *self,void *block8)
#ifdef _C_BLOWFISH_BUILTIN
  {
...
void Blowfish_Decrypt8_(C_BLOWFISH *self, uint_t *Lx, uint_t *Rx)
#ifdef _C_BLOWFISH_BUILTIN
  {
    uint_t  L = *Lx;
    uint_t  R = *Rx;
...
void Blowfish_Decrypt8(C_BLOWFISH *self,void *block8)
#ifdef _C_BLOWFISH_BUILTIN
  {
    uint_t L = Four_To_Unsigned_BE((byte_t*)block8+0);
    uint_t R = Four_To_Unsigned_BE((byte_t*)block8+4);
...
void Blowfish_Destruct(C_BLOWFISH *self)
#ifdef _C_BLOWFISH_BUILTIN
  {
    __Destruct(self);
  }
...
void *Blowfish_Init_Static(C_BLOWFISH *self, void *key, int key_len) 
#ifdef _C_BLOWFISH_BUILTIN
  {
    int i, j, k;
    uint_t data;
...
void *Blowfish_Init(void *key,int key_len) 
#ifdef _C_BLOWFISH_BUILTIN
  {
    ...
    C_BLOWFISH *self = __Object(sizeof(C_BLOWFISH),funcs);
...
void Blowfish_Encrypt_Bytes(void *data, int len, void *key, int key_len)
#ifdef _C_BLOWFISH_BUILTIN
  {
    int i;
    C_BLOWFISH bfctx = {{0},{{0},{0}}};
    Blowfish_Init_Static(&bfctx,key,key_len);
    
    if ( len % 8 ) __Raise(C_ERROR_UNALIGNED,0);
    for ( i = 0; i < len; i += 8 )
      {
        Blowfish_Encrypt8(&bfctx,(byte_t*)data+i);
      }
  }
#endif
  ;

void Blowfish_Encrypt(void *data, int len, char *passwd)
#ifdef _C_BLOWFISH_BUILTIN
  {
    byte_t key[16] = {0};
    Md5_Digest(passwd,strlen(passwd),key);
    Blowfish_Encrypt_Bytes(data,len&~7,key,sizeof(key));
  }
#endif
  ;

void Blowfish_Decrypt_Bytes(void *data, int len, void *key, int key_len)
#ifdef _C_BLOWFISH_BUILTIN
  {
    int i;
    C_BLOWFISH bfctx = {{0},{{0},{0}}};
    ...
    for ( i = 0; i < len; i += 8 )
      {
        Blowfish_Decrypt8(&bfctx,(byte_t*)data+i);
      }
  }
#endif
  ;

void Blowfish_Decrypt(void *data, int len, char *passwd)
#ifdef _C_BLOWFISH_BUILTIN
  {
    byte_t key[16] = {0};
    Md5_Digest(passwd,strlen(passwd),key);
    Blowfish_Decrypt_Bytes(data,len&~7,key,sizeof(key));
  }
#endif
  ;
