...
void Aes_Destruct(C_AES *ctx)
#ifdef _C_AES_BUILTIN
  {
    __Destruct(ctx);
  }
#endif
...
void *Aes_Init_Encipher_Static(C_AES *ctx, void *key, int key_len_bits)
#ifdef _C_AES_BUILTIN
  {
    int i;
    uint_t *RK;
    ...
    switch(key_len_bits)
      {
        ...
        default:
        __Raise_Format(C_ERROR_UNSUPPORTED,("unsupported key length %d bit",key_len_bits));
      }
    ...
    switch( ctx->nr )
      {
        case 10:
          for( i = 0; i < 10; i++, RK += 4 )
            {
              RK[4]  = RK[0] ^ C_AES_RCON[i] ^
              ( (uint_t) C_AES_FSb[ ( RK[3] >>  8 ) & 0x0ff ]       ) ^
              ( (uint_t) C_AES_FSb[ ( RK[3] >> 16 ) & 0x0ff ] <<  8 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[3] >> 24 ) & 0x0ff ] << 16 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[3]       ) & 0x0ff ] << 24 );
        ...
        case 12:
          for( i = 0; i < 8; i++, RK += 6 )
            {
              RK[6]  = RK[0] ^ C_AES_RCON[i] ^
              ( (uint_t) C_AES_FSb[ ( RK[5] >>  8 ) & 0x0ff ]       ) ^
              ( (uint_t) C_AES_FSb[ ( RK[5] >> 16 ) & 0x0ff ] <<  8 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[5] >> 24 ) & 0x0ff ] << 16 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[5]       ) & 0x0ff ] << 24 );
        ...
        case 14:
          for( i = 0; i < 7; i++, RK += 8 )
            {
              RK[8]  = RK[0] ^ C_AES_RCON[i] ^
              ( (uint_t) C_AES_FSb[ ( RK[7] >>  8 ) & 0x0ff ]       ) ^
              ( (uint_t) C_AES_FSb[ ( RK[7] >> 16 ) & 0x0ff ] <<  8 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[7] >> 24 ) & 0x0ff ] << 16 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[7]       ) & 0x0ff ] << 24 );
              ...
              RK[12] = RK[4] ^
              ( (uint_t) C_AES_FSb[ ( RK[11]       ) & 0x0ff ]       ) ^
              ( (uint_t) C_AES_FSb[ ( RK[11] >>  8 ) & 0x0ff ] <<  8 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[11] >> 16 ) & 0x0ff ] << 16 ) ^
              ( (uint_t) C_AES_FSb[ ( RK[11] >> 24 ) & 0x0ff ] << 24 );
...
void *Aes_Init_Decipher_Static(C_AES *ctx, void *key, int key_len_bits)
#ifdef _C_AES_BUILTIN
  {
    int i,j;
    uint_t *RK, *SK;
    ...
    *RK++ = *SK++;
    ...
    for( i = ctx->nr - 1, SK -= 8; i > 0; i--, SK -= 8 )
    {
        for( j = 0; j < 4; j++, SK++ )
        {
            *RK++ = C_AES_RT0[ C_AES_FSb[ ( *SK       ) & 0x0ff ] ] ^
                    C_AES_RT1[ C_AES_FSb[ ( *SK >>  8 ) & 0x0ff ] ] ^
                    C_AES_RT2[ C_AES_FSb[ ( *SK >> 16 ) & 0x0ff ] ] ^
                    C_AES_RT3[ C_AES_FSb[ ( *SK >> 24 ) & 0x0ff ] ];
        }
    }
...
#define FROUND(X0,X1,X2,X3,Y0,Y1,Y2,Y3) \
  { \
    X0 = *RK++ ^ C_AES_FT0[ ( Y0       ) & 0x0ff ] ^\
                 C_AES_FT1[ ( Y1 >>  8 ) & 0x0ff ] ^\
                 C_AES_FT2[ ( Y2 >> 16 ) & 0x0ff ] ^\
                 C_AES_FT3[ ( Y3 >> 24 ) & 0x0ff ]; \
\
    X1 = *RK++ ^ C_AES_FT0[ ( Y1       ) & 0x0ff ] ^\
                 C_AES_FT1[ ( Y2 >>  8 ) & 0x0ff ] ^\
                 C_AES_FT2[ ( Y3 >> 16 ) & 0x0ff ] ^\
                 C_AES_FT3[ ( Y0 >> 24 ) & 0x0ff ]; \
\
    X2 = *RK++ ^ C_AES_FT0[ ( Y2       ) & 0x0ff ] ^\
                 C_AES_FT1[ ( Y3 >>  8 ) & 0x0ff ] ^\
                 C_AES_FT2[ ( Y0 >> 16 ) & 0x0ff ] ^\
                 C_AES_FT3[ ( Y1 >> 24 ) & 0x0ff ]; \
\
    X3 = *RK++ ^ C_AES_FT0[ ( Y3       ) & 0x0ff ] ^\
                 C_AES_FT1[ ( Y0 >>  8 ) & 0x0ff ] ^\
                 C_AES_FT2[ ( Y1 >> 16 ) & 0x0ff ] ^\
                 C_AES_FT3[ ( Y2 >> 24 ) & 0x0ff ]; \
  }

void Aes_Encrypt16( C_AES *ctx, void *block16 )
#ifdef _C_AES_BUILTIN
  {
    int i;
    ...
    X0 = *RK++ ^ \
            ( (uint_t) C_AES_FSb[ ( Y0       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_FSb[ ( Y1 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_FSb[ ( Y2 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_FSb[ ( Y3 >> 24 ) & 0x0ff ] << 24 );

    X1 = *RK++ ^ \
            ( (uint_t) C_AES_FSb[ ( Y1       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_FSb[ ( Y2 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_FSb[ ( Y3 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_FSb[ ( Y0 >> 24 ) & 0x0ff ] << 24 );

    X2 = *RK++ ^ \
            ( (uint_t) C_AES_FSb[ ( Y2       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_FSb[ ( Y3 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_FSb[ ( Y0 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_FSb[ ( Y1 >> 24 ) & 0x0ff ] << 24 );

    X3 = *RK++ ^ \
            ( (uint_t) C_AES_FSb[ ( Y3       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_FSb[ ( Y0 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_FSb[ ( Y1 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_FSb[ ( Y2 >> 24 ) & 0x0ff ] << 24 );
...
#define RROUND(X0,X1,X2,X3,Y0,Y1,Y2,Y3) \
  { \
    X0 = *RK++ ^ C_AES_RT0[ ( Y0       ) & 0x0ff ] ^\
                 C_AES_RT1[ ( Y3 >>  8 ) & 0x0ff ] ^\
                 C_AES_RT2[ ( Y2 >> 16 ) & 0x0ff ] ^\
                 C_AES_RT3[ ( Y1 >> 24 ) & 0x0ff ]; \
\
    X1 = *RK++ ^ C_AES_RT0[ ( Y1       ) & 0x0ff ] ^\
                 C_AES_RT1[ ( Y0 >>  8 ) & 0x0ff ] ^\
                 C_AES_RT2[ ( Y3 >> 16 ) & 0x0ff ] ^\
                 C_AES_RT3[ ( Y2 >> 24 ) & 0x0ff ]; \
\
    X2 = *RK++ ^ C_AES_RT0[ ( Y2       ) & 0x0ff ] ^\
                 C_AES_RT1[ ( Y1 >>  8 ) & 0x0ff ] ^\
                 C_AES_RT2[ ( Y0 >> 16 ) & 0x0ff ] ^\
                 C_AES_RT3[ ( Y3 >> 24 ) & 0x0ff ]; \
\
    X3 = *RK++ ^ C_AES_RT0[ ( Y3       ) & 0x0ff ] ^\
                 C_AES_RT1[ ( Y2 >>  8 ) & 0x0ff ] ^\
                 C_AES_RT2[ ( Y1 >> 16 ) & 0x0ff ] ^\
                 C_AES_RT3[ ( Y0 >> 24 ) & 0x0ff ]; \
  }

void Aes_Decrypt16( C_AES *ctx, void *block16 )
#ifdef _C_AES_BUILTIN
  {
    int i;
    uint_t *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
    ...
    X0 = *RK++ ^ \
            ( (uint_t) C_AES_RSb[ ( Y0       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_RSb[ ( Y3 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_RSb[ ( Y2 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_RSb[ ( Y1 >> 24 ) & 0x0ff ] << 24 );

    X1 = *RK++ ^ \
            ( (uint_t) C_AES_RSb[ ( Y1       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_RSb[ ( Y0 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_RSb[ ( Y3 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_RSb[ ( Y2 >> 24 ) & 0x0ff ] << 24 );

    X2 = *RK++ ^ \
            ( (uint_t) C_AES_RSb[ ( Y2       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_RSb[ ( Y1 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_RSb[ ( Y0 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_RSb[ ( Y3 >> 24 ) & 0x0ff ] << 24 );

    X3 = *RK++ ^ \
            ( (uint_t) C_AES_RSb[ ( Y3       ) & 0x0ff ]       ) ^
            ( (uint_t) C_AES_RSb[ ( Y2 >>  8 ) & 0x0ff ] <<  8 ) ^
            ( (uint_t) C_AES_RSb[ ( Y1 >> 16 ) & 0x0ff ] << 16 ) ^
            ( (uint_t) C_AES_RSb[ ( Y0 >> 24 ) & 0x0ff ] << 24 );
...
void *Aes_Init_Encipher(void *key,int key_len)
#ifdef _C_AES_BUILTIN
  {
    ...
    C_AES *ctx = __Object(sizeof(C_AES),funcs);
...
void Aes_Encrypt_Bytes(void *data, int len, void *key, int keylen)
#ifdef _C_AES_BUILTIN
  {
    int i;
    C_AES actx = {0,0,{0}};
    if ( len % 16 ) __Raise(C_ERROR_UNALIGNED,0);
    Aes_Init_Encipher_Static(&actx,key,keylen);

    for ( i = 0; i < len; i += 16 )
      {
        Aes_Encrypt16(&actx,(byte_t*)data+i);
      }
  }
#endif
  ;

void Aes_Encrypt(void *data, int len, char *S)
#ifdef _C_AES_BUILTIN
  {
    byte_t key[32];
    Sha2_Digest_Digest(S,strlen(S),key);
    Aes_Encrypt_Bytes(data,len&~15,key,sizeof(key));
  }
#endif
  ;

void Aes_Decrypt_Bytes(void *data, int len, void *key, int keylen)
#ifdef _C_AES_BUILTIN
  {
    int i;
    C_AES actx = {0,0,{0}};
    if ( len % 16 ) __Raise(C_ERROR_UNALIGNED,0);
    Aes_Init_Decipher_Static(&actx,key,keylen);

    for ( i = 0; i < len; i += 16 )
      {
        Aes_Decrypt16(&actx,(byte_t*)data+i);
      }
  }
#endif
  ;

void Aes_Decrypt(void *data, int len, char *S)
#ifdef _C_AES_BUILTIN
  {
    byte_t key[32];
    Sha2_Digest_Digest(S,strlen(S),key);
    Aes_Decrypt_Bytes(data,len&~15,key,sizeof(key));
  }
#endif
  ;
