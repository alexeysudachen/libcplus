...
#ifdef _BUILTIN
#define _C_SHA1_BUILTIN
#endif

#include "C+.hc"
#include "crc.hc"

typedef struct _C_SHA1
  {
...
void *Sha1_Clone(C_SHA1 *sha1)
#ifdef _C_SHA1_BUILTIN
  {
    return __Clone(sizeof(C_SHA1),sha1);
  }
#endif
  ;
...
void Sha1_Update(C_SHA1 *sha1, void *data, int len);
...
void *Sha1_Init()
#ifdef _C_SHA1_BUILTIN
  {
    ...
    C_SHA1 *sha1 = __Object(sizeof(C_SHA1),funcs);
...
#ifdef _C_SHA1_BUILTIN
  {
    C_SHA1 sha1 = C_SHA1_INITIALIZER;
...
#define Sha1_Digest_Of(Data,Len) Sha1_Digest(Data,Len,0)
  ...
  void Sha1_Internal_Transform(C_SHA1 *sha1, void *block)
    {
  ...
  void Sha1_Update(C_SHA1 *sha1, void const *input, int input_length)
    {      
      int i, index, partLen;
...
#endif /* _C_SHA1_BUILTIN */

typedef struct _C_HMAC_SHA1
  {
    C_SHA1 sha1;
    byte_t ipad[64];
    byte_t opad[64];
...
void *Hmac_Sha1_Clone(C_HMAC_SHA1 *hmac)
#ifdef _C_SHA1_BUILTIN
  {
    return __Clone(sizeof(C_HMAC_SHA1),hmac);
  }
#endif
  ;

void *Hmac_Sha1_Start(C_HMAC_SHA1 *hmac, const void *key, int key_len)
#ifdef _C_SHA1_BUILTIN
  {
    int i;
    byte_t sum[20];
...
void Hmac_Sha1_Update(C_HMAC_SHA1 *hmac, const void *input, int input_length)
#ifdef _C_SHA1_BUILTIN
  {
...
void *Hmac_Sha1_Finish(C_HMAC_SHA1 *hmac, void *digest)
#ifdef _C_SHA1_BUILTIN
  {
    byte_t tmpb[20];
...
void Hmac_Sha1_Reset(C_HMAC_SHA1 *hmac)
#ifdef _C_SHA1_BUILTIN
  {
    Sha1_Start(&hmac->sha1);
    Sha1_Update(&hmac->sha1,hmac->ipad,64);
...
void *Hmac_Sha1_Init(void *key, int key_len)
#ifdef _C_SHA1_BUILTIN
  {
    ...
    C_HMAC_SHA1 *sha1 = __Object(sizeof(C_HMAC_SHA1),funcs);
